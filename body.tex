...
\begin{figure}[h t b p]
  ...
  \caption{A diamond2 dependent \gls{computational graph}}
  \label{diamond2}
\end{figure}
...
\begin{figure}[h t b p]
  ...
  digraph {
    ...
    softmax -> classifiers -> reLU -> fc_1 -> reLU -> fc_0

    
  }
  \end{dot2tex}
  \caption{The models used for benchmarks}
  \label{benchmark architecture2}
\end{figure}
...
Those benchmarks are built with Jmh \cite{shipilevjmh}, measuring the number of mini-batches per second, for training neural networks that contain variant number of branches of sub-networks, as classifiers for CIFAR-100 dataset \cite{krizhevsky2009learning}. The architecture of the network used for benchmarks summarized in Figure~\ref{benchmark architecture}.
