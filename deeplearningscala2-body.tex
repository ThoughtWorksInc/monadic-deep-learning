...
However, the type signature of \lstinline{layer2} seems not very strange. It accepts a \lstinline{hyperparameters1.INDArrayLayer} as input and returns a \lstinline{hyperparameters2.INDArrayLayer}. The type signature means that the previous layer of \lstinline{layer2} must be another layer whose hyperparameters are configured in \lstinline{hyperparameters1}.
...
\begin{itemize}
  ...
  \item An ordinary vector input. i.e. \lstinline{INDArray}.

  \item Hidden states produced by any previous neural network layers, regardless of what hyperparameters are configured for those layers. i.e. any \lstinline{INDArrayLayer}s regardless of the prefixes.
  
  \item \Glspl{trainable variable} in the case of generative neural network like DeepDream\cite{mordvintsev2015deepdream}.i.e. any \lstinline{INDArrayWeight}s regardless of the prefixes.
