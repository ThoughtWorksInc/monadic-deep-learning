...
\begin{lstlisting}[caption={Impossible Transform Function for Automatic Differentiation}, label={transform}]
...
\begin{lstlisting}[caption={Type Safe Transform Function for Automatic Differentiation}, label={typeSafeTransform}]
...
The remainder of Section \ref{backpropagation} is organized as follows.
% TODO:

\subsection{Multiple Trainable Variables}
...
\begin{lstlisting}[caption={Dual number for forward mode AD}, label={DualNumber}]
type Data = Double
...
\begin{lstlisting}[caption={Arithmetic operations on dual number}, label={dualArithmetic}]
object DualNumber {
...
\begin{lstlisting}[caption={Replacing \lstinline{PartialDelta} to a closure}, label={class ClosureBasedDualNumber}]
type Data = Double  
case class ClosureBasedDualNumber(data: Data, delta: UpdateWeights)
\end{lstlisting}
...
\begin{lstlisting}[caption={Replacing operations on \lstinline{PartialDelta} to custom functions for \lstinline{UpdateWeights}}, label={object ClosureBasedDualNumber}]  

object ClosureBasedDualNumber {
  def plus(left: ClosureBasedDualNumber, right: ClosureBasedDualNumber): ClosureBasedDualNumber = {
    ClosureBasedDualNumber(left.data + right.data, UpdateWeights.plus(left.delta, right.delta))
  }
  def multiply(left: ClosureBasedDualNumber, right: ClosureBasedDualNumber): ClosureBasedDualNumber = {
    ClosureBasedDualNumber(
      left.data * right.data,
      UpdateWeights.multiply(left.data, right.delta) + UpdateWeights.multiply(right.data, left.delta))
  }
}
\end{lstlisting}


Mathematically, the \lstinline{UpdateWeights} type in a dual number can be any vector space, i.e. the \lstinline{UpdateWeights} closure itself must support addition and scalar multiplication operations(Listing \ref{UpdateWeights}).

\begin{lstlisting}[caption={Arithmetic operations for the closure that contains side-effects}, label={UpdateWeights}, escapeinside={(*}{*)}]
type UpdateWeights = Double => SideEffects
object UpdateWeights {
  /**(* $(f_0 + f_1)(x) = f_0(x) + f_1(x)$ *)*/
  def plus(f0: UpdateWeights, f1: UpdateWeights) = { x: Double =>
    f0(x) |+| f1(x)
  }

  /**(* $(x_0f)(x_1) = f(x_0x_1)$ *)*/
  def multiply(x0: Double, f: UpdateWeights) = { x1: Double =>
    f(x0 * x1)
  }
}
\end{lstlisting}
...
\lstinline{|+|} is the \lstinline{append} operation of \lstinline{scala.Semigroup}, which could be any cumulative data type. In DeepLearning.scala, our \lstinline{SideEffects} is based on the asynchronous operation \lstinline{UnitContinuation}\footnote{\lstinline{UnitContinuation[A]} is an opaque alias\cite{erik2017opaque} of \lstinline{(A => Trampoline[Unit]) => Trampoline[Unit]}, implemented as a separate library at \url{https://github.com/ThoughtWorksInc/future.scala}. In DeepLearning.scala it is used as a monadic data type for encapsulating side effects in stack-safe asynchronous programming.}.

\begin{lstlisting}[caption={Monadic side-effects}, label={SideEffects}]
type SideEffects = UnitContinuation[Unit]
\end{lstlisting}

The \lstinline{SideEffects} for neural networks conform associative law because the side effects are just updating \glspl{trainable variable}. Thus, our \lstinline{UpdateWeights.plus} and \lstinline{UpdateWeights.multiply} are equivalent to the operations on strictly evaluated scalar value \lstinline{PartialDelta} in forward mode AD.

\subsection{Diamond Dependencies in Differentiable Expressions}

Although the closured-based dual number approach from Listing \ref{class ClosureBasedDualNumber} to \ref{SideEffects} supports multiple \glspl{trainable variable}, the closure-based computation has a performance issue in the case of diamond dependencies.

\begin{lstlisting}[caption={Monadic side-effects}, label={y1}]  
var weight: Double = math.random
val learningRate = 0.001

def readWeight() = ClosureBasedDualNumber(weight, { delta =>
  weight -= learningRate * delta
})

val w = readWeight()
val y0 = ClosureBasedDualNumber.multiply(w, w)
...
% returns the side-effects for updating weights according to a coefficient that 


% Those arithmetic operations can be implemented if \lstinline{UpdateWeights} has an additional parameter:

 
% There are two ways to understand \lstinline{UpdateWeights.plus} and \lstinline{UpdateWeights.multiply} in Listing \ref{UpdateWeights}.
...
% we don't need 
% In order to type-check the \lstinline{PartialDelta}, we 

% Our approach is 


% \begin{lstlisting}
% case class Tape[Data, PartialDelta](data: Data, backward: PartialDelta => Patch)
% \end{lstlisting}


% As we discussed in Section \ref{introduction}, automatic differentiation algorithms for neural network should support multiple \glspl{trainable variable}. That means our automatic differentiation primitive should return a function that computes all derivatives referenced by \lstinline{differentiableFunction}, instead of the derivative for input.

% % our automatic differentiation primitive

% Hence, Listing \ref{typeSafeTransform} is not enough. We have to change the type signature of our automatic differentiation primitive to:
